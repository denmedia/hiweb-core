...
if(!function_exists('hiweb')) :
    function hiweb(){ static $hiweb = null; if(is_null($hiweb)) $hiweb = new hiweb(); return $hiweb; } endif;
...
if(!class_exists('hiweb')) :
     ...
     * @method hiweb_cms cms() cms() Подключение субКласса hiweb_cms. Подключение к CMS
     * Class hiweb
     */
    class hiweb {
...
////////////////////////////////////////////////////
        ...
        public function tpl(){
            static $class = null;
            if(!is_object($class) && file_exists(dirname(__FILE__).'/hiweb-tpl.php')) {
        ...
        public function error($showBacktrace = false){
            static $class = null;
            if(!is_object($class) && file_exists(dirname(__FILE__).'/hiweb-error.php')) { require_once dirname(__FILE__) . '/hiweb-error.php'; $class = new hiweb_error($showBacktrace); }
            return $class;
        }
...
//////////////////////////////////////////////////////
        ...
        public function request($keyMix, $def = null){
            return $this->array()->getVal($_REQUEST, $keyMix, $def);
        }
         ...
         * @return mixed
         */
        public function getVal_fromArr($haystack = array(), $keyMix = '', $def = null){ return $this->array()->getVal($haystack, $keyMix, $def); }
        ...
        public function debug($showBacktrace = false){
            ...
            $this->console()->warn('hiweb->debug включен : '. $this->file()->getStr_linkPath($this->array()->getVal($debugBacktrace, array(0,'file'))).' : line:'.$this->array()->getVal($debugBacktrace, array(0,'line')));
        }
        ...
        public function getArr_clientInfo() {
            if( isset($_SESSION['client']) && is_array($_SESSION['client']) && count($_SESSION['client']) > 0 ) { return $_SESSION['client']; }
            if(!isset($_SERVER['HTTP_USER_AGENT'])) { return; }
            $u_agent = $_SERVER['HTTP_USER_AGENT']; $bname = ''; $platform = ''; $version= "";
            ///
            ...
            if ($i != 1) {
                if (strripos($u_agent,"Version") < strripos($u_agent,$ub)){ $version= isset($matches['version'][0]) ? $matches['version'][0] : $matches['version']; }
        ...
        public function getBool_mobileBrowser() {
            if( isset($_SESSION['client']['mobile']) ) { return $_SESSION['client']['mobile']; }
        ...
        public function getArr_debugBacktrace($class = true, $functions = true, $files = true, $dirs = true, $paths = true, $returnChunkArray = false, $minDepth = 2, $maxDepth = 4, $prepend = '', $append = '', $args = false){
            ...
            foreach($dbt as $i){
                ...
                if($dirs && isset($i['file']))              {$r['dirName'][] = $prepend.basename(dirname($i['file'])).$append; if($n < $maxDepth) $r['dirName'][] = $prepend.basename(dirname(dirname($i['file']))).$append;}
        ...
        public function getStr_debugBacktraceFunctionLocate($depth = 1){
            ...
            else return $this->file()->getStr_linkPath($this->array()->getVal($debugBacktrace, array($depth,'file'), ':файл не найден:')).' : '.$this->array()->getVal($debugBacktrace, array($depth,'line'));
        }
        ...
        public function getStr_debugBacktraceFunctionTrace($depth = 1){
            $debugBacktrace = debug_backtrace();
            $class = $this->array()->getVal($debugBacktrace, array($depth,'class'),'');
            $function = $this->array()->getVal($debugBacktrace, array($depth,'function'),'');
            $type = $this->array()->getVal($debugBacktrace, array($depth,'type'),'');
            //Class filter
            if(strpos($class,'hiweb_')===0 && method_exists(hiweb(),substr($class,6))) $r = 'hiweb->'.substr($class,6);
            else $r = $class;
            $r .= $type.$function;
            return $r;
        }
        ...
        public function cache($result = null){
            ...
            $keyArr = array();
            if(!is_null($this->array()->getVal($dbArr,array('path',0)))) $keyArr[] = $this->array()->getVal($dbArr,array('path',0));
            if(!is_null($this->array()->getVal($dbArr,array('className',0)))) $keyArr[] = $this->array()->getVal($dbArr,array('className',0));
            if(!is_null($this->array()->getVal($dbArr,array('functionName',0)))) $keyArr[] = $this->array()->getVal($dbArr,array('functionName',0));
            if(!is_null($this->array()->getVal($dbArr,array('args',0)))) $keyArr[] = md5(json_encode($this->array()->getVal($dbArr,array('args',0))));
            $key = implode(':',$keyArr);
            if(is_null($this->array()->getVal($this->_cache,$key))) { $this->_cache[$key] = $result; return $result; }
            else { return $this->array()->getVal($this->_cache,$key); }
        }
        ...
        public function cacheExists(){
            ...
            if(!is_null($this->array()->getVal($dbArr,array('args',0)))) $keyArr[] = md5(json_encode($this->array()->getVal($dbArr,array('args',0))));
            $key = implode(':',$keyArr);
            return !is_null($this->array()->getVal($this->_cache,$key));
        }
        ...
        public function cacheByFile($result = null){
            ...
            if(is_null($this->array()->getVal($this->_cacheByFile,$key))) { return $result; }
            else { return $this->array()->getVal($this->_cacheByFile,$key); }
        }
        ...
        public function cacheByFileExists(){
            ...
            if(!is_null($this->array()->getVal($dbArr,array('args',0)))) $keyArr[] = md5(json_encode($this->array()->getVal($dbArr,array('args',0))));
            $key = implode(':',$keyArr);
            return !is_null($this->array()->getVal($this->_cacheByFile,$key));
        }
